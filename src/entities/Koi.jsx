...
function quatFromDirNoRoll(dir, out) {
  ...
  const EPS = 1e-3;
  const MAX_PITCH = Math.PI * 0.1 - EPS; // < 90°
...
function Koi({
  ...
  const midY = (depth[0] + depth[1]) * 0.5
  const { point: mousePoint, has: mouseHas } = useMouseOnPlane(midY, pondRadius * 0.95)

  const stateKey = `koi`

  const persistent = useMemo(() => getBoidState(stateKey, () => ({
    pos: new Float32Array(count * 3),
    vel: new Float32Array(count * 3),
    scl: new Float32Array(count),
    initialized: false,
  })), [stateKey, count])

  const pos = useRef(persistent.pos)
  const vel = useRef(persistent.vel)
  const scl = useRef(persistent.scl)
  ...
  const patchKoiMaterial = useMemo(() => {
    return (material, lutObj) => {
      ...
      material.onBeforeCompile = ((prev) => (shader) => {
        ...
        // new controls
        // "small" (less than 30%) is red
        const smallCutoff = (scaleRange[0] + scaleRange[1]) * 0.3 // midpoint; tweak if you want
        shader.uniforms.uSmallCutoff = { value: smallCutoff }
        // 2% * 70% chance for gold
        ...
        shader.vertexShader = shader.vertexShader
          ...
          vScale = aScale; // pass scale per instance
        `)
        ...
        shader.fragmentShader = shader.fragmentShader
          ...
          // deterministic hash from instanceId in [0,1)
          float hash1(float n) {
  ...
  useEffect(() => {
    ...
    if (!persistent.initialized) {
      ...
      setBoidState(stateKey, persistent)
    }

    // Write instance matrices from current pos/vel/scl
    for (let i = 0; i < count; i++) {
      const ix = 3 * i
      const x = p[ix + 0], y = p[ix + 1], z = p[ix + 2]
      d.set(vel.current[ix + 0], vel.current[ix + 1], vel.current[ix + 2])
      if (d.lengthSq() < 1e-8) {
        // safety: if somehow zero, pick a horizontal heading
        const yaw = Math.random() * Math.PI * 2
        d.set(Math.sin(yaw), 0, Math.cos(yaw))
      }
      quatFromDirNoRoll(d, q)
  ...
  useFrame((_, dt) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (mouseHas.current) {
        ...
        const cutoffNear = 1.0   // full suppression inside 1 unit
        const cutoffFar = 3.0   // full strength beyond 3 units
    ...
    // Write transforms to instances
    for (let i = 0; i < count; i++) {
      const ix = 3 * i
      const px = p[ix + 0], py = p[ix + 1], pz = p[ix + 2]
      dir.set(v[ix + 0], v[ix + 1], v[ix + 2]).normalize()
      if (dir.lengthSq() > 0) {
        quatFromDirNoRoll(dir, quat);
      } else {
        quat.identity()
      }
